...
class CourseTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.course = Course.objects.create(
            course_name="Python", description="Хороший курс}", owner=self.user
        )
        self.lesson = Lesson.objects.create(
            lesson_name="DRF", course_name=self.course, owner=self.user
        )
    ...
    def test_course_update(self):
        ...
        data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(data.get("course_name"), "Python")
...
class LessonTestCase(APITestCase):
    ...
    def test_lesson_update(self):
        ...
        data = {"lesson_name": "Postgres"}
        response = self.client.patch(url, data)
        data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(data.get("lesson_name"), "Postgres")
